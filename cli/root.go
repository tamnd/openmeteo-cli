// Package cli assembles the openmeteo command tree from the openmeteo
...
// NewApp assembles the kit application from the openmeteo domain. The
...
//
// To add a command, declare it in openmeteo/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := openmeteo.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(openmeteo.Domain{}).Register(app)
