...
describes what users get. Pick a format with `-o`, or let openmeteo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
openmeteo <command> -o table   # aligned columns for reading
openmeteo <command> -o jsonl   # one JSON object per line, for piping
openmeteo <command> -o json    # a single JSON array
openmeteo <command> -o csv     # spreadsheet friendly
openmeteo <command> -o tsv     # tab-separated
openmeteo <command> -o url     # just the URL column
openmeteo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
openmeteo <command> --fields id,title,url
```
...
```bash
openmeteo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
openmeteo <command>            # a table, because this is a terminal
openmeteo <command> | wc -l    # JSONL, because this is a pipe
```
